...
```{r}
spooky_wrd <- unnest_tokens(spooky, word, text) # gives us table with one word per row
head(spooky_wrd)

head(stop_words) # collection of words that are boring
tail(stop_words)
spooky_wrd <- anti_join(spooky_wrd, stop_words, by = "word") # if you see a word in spookywrd that matches a stop word, get rid of it. 
head(spooky_wrd)
````
...
multiplot(p1, p2, p3, layout = layout) # to use this command, need it in lib folder.
```
From the above plots we find:

* EAP is featured most frequently in dataset. About 8000 of sentences belong to him. 

* Sentence length is more variable for EAP than the others. Has shorter sentences. 

* EAP has slightly longer words than others. 
...
head(freqs) # look at how often the top appear
tail(freqs)
head(sort(freqs, decreasing = TRUE))
png("../figs/Workcloud_all.png") # This is how you save it in the figs folder, working directory needs to be in doc folder
wordcloud(words, freqs, max.words = 50, color = c("purple4", "red4", "black"))
dev.off() # ends connection to figs folder 
```
...
head(tf_idf) # Product of term frequency and inverse frequency. Counted # of times each word appears for each author than can measure these fore each word. 
tail(tf_idf)
...
tf_idf_30 <- top_n(tf_idf, 30, tf_idf) # choosing 30 words with highest TFIDF scores
...
**tf is number of time word appeared/number of words in corpus. idf measures relative to others.** 

Below are the top 20 terms for each author. Terms that MWS uses frequently that aren't used as much by others are the ones in blue, etc. 
```{r}
...
From *[Text Mining with R; A Tidy Approach](https://www.tidytextmining.com)*, "When human readers approach text, we use our understanding of the emotional intent of words to infer whether a section of text is positive or negative, or perhaps characterized by some other more nuanced emotion like surprise or disgust.  We can also use the tools of text mining to approach the emotional content of text programmatically."  This is the goal of sentiment analysis. Can we come up with an alogirthm that elicits some emotion in reader? 
```{r}
# Keep words that have been classified within the NRC lexicon.
get_sentiments('nrc') # this is code that will list the words that are in this nrc lexicon along with the sentiment associated with it. 
sentiments <- inner_join(spooky_wrd, get_sentiments('nrc'), by = "word") # this made sure there was one word per line and dropped any words that didn't have sentiments 
head(sentiments)
count(sentiments, sentiment) # records freq of each sentiment for each author
...
+ Used a facet-wrap to create plot so indices are the same

+ Mary Shelley uses more words that are in our lexicon. Maybe uses more emotional words, but has a higher freq of words that are found in our lexicon. 

+ EAP and Mary have more positive than negative words, but not HPL. 

+ Fear and sadness are some of top emotions; maybe to be expected from horror fiction. 
...
# then throw away words that aren't associated with positive sentiment
positive <- inner_join(spooky_wrd, nrc_pos, by = "word")
head(positive) # same spooky word but now only those that have pos. sentiment 
...
+ MWS used the word "love" way more than others. 
+ Good to focus in on words and emotions that authors differ in
...
# Creates a DTM matrix -- needs to be in this format instead of tidy text . Row for each sentence, column for each word. Will tell how many times that word appeared in a sentence.
...
```{r}
spooky_wrd_lda    <- LDA(spooky_wrd_tm, k = 12, control = list(seed = 1234)) # get a per topic, per word probability matrix
...
For each topic, tells me the probability this word generates this topic. 
