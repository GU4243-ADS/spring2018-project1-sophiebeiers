...
### Introduction
The goal of this project is to analyze the work of three "spooky" authors: H.P. Lovecraft (`HPL`), Mary Shelly (`MWS`), and Edgar Allen Poe (`EAP`). Below, I assess their similarities and differences in semantics, common words and themes and length of work. I end by zooming in on MWS's work and her use of the word "love."
...
png("~/Documents/GitHub/spring2018-project1-sophiebeiers/figs/pronounplot.png") 
...
# Visualizations
png("~/Documents/GitHub/spring2018-project1-sophiebeiers/figs/swlengths.png")
...
### Analysis
#### Top 10 Words of Each Author
I visualized each author's top ten words used in his/her work.
```{r, message = FALSE, warning = FALSE}
# Top 10 Words Dataframes
top10 <- spooky_wrd_clean %>%
         filter(rank <= 10)

png("~/Documents/GitHub/spring2018-project1-sophiebeiers/figs/top10.png")
top10 %>% 
...
```{r}
# HPL wordcloud
png("~/Documents/GitHub/spring2018-project1-sophiebeiers/figs/hplWC.png")
...
```{r}
# EAP wordcloud
png("~/Documents/GitHub/spring2018-project1-sophiebeiers/figs/eapWC.png")
...
spooky_wrd_clean %>% 
  ...
  theme_light()
```
First, I plotted the relationship between the author's most frequently used words and their rank. 
We can see that the relationship between each word's rank and frequency has a negative slope and that all three authors' work follows this pattern. Then, I plot the tf-idfs to see which words are most important to each author.
...
png("~/Documents/GitHub/spring2018-project1-sophiebeiers/figs/tfidf.png")
idftf %>%
  arrange(desc(tf_idf)) %>%
...
png("~/Documents/GitHub/spring2018-project1-sophiebeiers/figs/posneghm.png")
...
Ironically, while MWS seems to be the most "positive" author, she still uses a quarter more negative words than positive words in her pieces; this leads me to think that it's possible she may be negating the positive words in a way that makes the overall sentiment actually *negative.* Thus, I turn to the "sentimentr" package to analyze multiple words at a time. 

####$ MWS 
The text in our spooky data is already split up into sentences, so I return to our original spooky data for the following analyses. 
```{r, warning = FALSE, message= FALSE}
## WHY THIS??
author_sentence_sent <- spooky %>%
  group_by(author) %>%
  mutate(sentence_n = 1:n(),
    index = round(sentence_n / n(), 2)) %>%
  unnest_tokens(word, text) %>%
  inner_join(get_sentiments("afinn")) %>%
  group_by(author, index) %>%
...
png("~/Documents/GitHub/spring2018-project1-sophiebeiers/figs/negwrds.png")
no_bigram %>%
...
```{r}
png("~/Documents/GitHub/spring2018-project1-sophiebeiers/figs/loveplts.png")
...
#### Prediction Analysis
Lastly, I use a linear model to try and predict the author of a batch of texts. First, I transform the tidy data into a document term matrix. 
```{r}
...
head(findFreqTerms(testdtm, lowfreq = 100)) # view terms that appear at least 100 times
findAssocs(testdtm, "life", corlimit = 0.5)
plot(testdtm, terms = findFreqTerms(testdtm, lowfreq = 100), 
     corThreshold = 0.05)

#testdtm <- as.matrix(testdtm)
list(which(test$author == "MWS"))

# Add Author Column
#dtm1 <- data.frame(as.matrix(testdtm))
#test <- spooky[,c("id","author")]
#merged <- merge(test, dtm1, by.x = "id", by.y = "row.names" )
#merged <- as.data.frame(merged)
#head(merged)
#merged <- as.numeric(merged[ ,3:25618])
#merged <- merged / rowSums(merged[ sapply(merged, is.numeric)]) * 1000

dtm.sparse <- sparseMatrix(testdtm$i, testdtm$j, x = testdtm$v, 
                           dims = c(testdtm$nrow, testdtm$ncol), 
                           dimnames = testdtm$dimnames)

dtm.sparse <- dtm.sparse/rowSums(dtm.sparse) * 1000


list(which(test$author == "MWS"))
#dtm.sparse <- merge(test, dtm.sparse, by.x = "id", by.y = "row.names" )

# TRied this, didn't like to classify 
dtm.lda <- LDA(weightTf(dtm.sparse), k = 3) # LDA function, 3 clusters
table <- round(head(posterior(dtm.lda, dtm.sparse)$topics), digits = 2) # create shortened table of probabilities 
table2 <- round((posterior(dtm.lda, dtm.sparse)$topics), digits = 2) # full table with all probabilities for own use
table

                    
# Training and Testing 
#author <- rep(NA, nrow(dtm1))
#author["MWS"] <- 1 # 1 if MWS
#author["EAP"] <- -1 # -1 if EAP

#in_train <- createDataPartition(y = merged$author.x, p = 3 / 4, list = FALSE)
#training <- merged[ in_train, ] 
#testing  <- merged[-in_train, ]

#fit <- lm(author.x ~ love + life, data = training)
#pred <- predict(fit, newdata = testing)

#table(testing$author.x, pred)
